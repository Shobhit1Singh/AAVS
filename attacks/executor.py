...
class TestExecutor:
    ...
    def __init__(
        ...
        Args:
            ...
            session_manager: Optional SessionManager for auth
        """
        self.base_url = base_url.rstrip('/')
        self.default_headers = headers or {}
        self.timeout = timeout

        # ⭐ KEY FIX: support both auth and non-auth modes
        if session_manager:
            self.session = session_manager.get_authenticated_session()
        else:
            self.session = requests.Session()

        # Apply default headers safely
    ...
    def execute_all_attacks(
        self, attacks, endpoint_path, method, delay=0.1
    ):
        results = []
