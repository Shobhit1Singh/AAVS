...
class ResponseAnalyzer:
    ...
    def analyze_result(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if status_code == 200 and result.get('attack_type') in ['SQL Injection', 'Command Injection']:
        ...
        if result.get('attack_type') == 'Missing Required Field' and status_code == 200:
            vulnerabilities_found.append({
                'type': 'Validation Bypass',
                'severity': 'MEDIUM',
                'reason': f'Required field "{result.get("param_name", "")}" was not enforced',
            })

        # Check 7: Boundary violations accepted
        if result.get('attack_type') == 'Boundary Violation' and status_code == 200:
            vulnerabilities_found.append({
        ...
        # Use rule engine to determine vulnerabilities
        rule_findings = VulnerabilityRules.run_all(result)
        if rule_findings:
            result["vulnerability_detected"] = True
            result["vulnerabilities"] = rule_findings
            self.vulnerabilities.append(result)
            logger.warning(
                f"{Fore.YELLOW}Vulnerability detected: {result.get('attack_type','UNKNOWN')} "
                f"on {result.get('param_name','UNKNOWN')} - {len(rule_findings)} issues{Style.RESET_ALL}"
            )
        else:
    ...
    def analyze_all_results(self, results: List[Dict]) -> List[Dict]:
        ...
        analyzed = [self.analyze_result(result) for result in results]
